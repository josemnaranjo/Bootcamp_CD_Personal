...
const listClientsIds = () => {
    const clientId = clients.map(client => client.id);
    return clientId;
};
...
const listClientsIdsSortByTaxNumber = () => {
    //ordernar por rut
    const rutsOrdenados = clients.sort((a,b) => parseFloat(a.taxNumber) - parseFloat(b.taxNumber));
    return listClientsIds(rutsOrdenados)
}
